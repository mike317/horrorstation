/obj/table
	...
	New()
		...
		if (ALIENMODE && prob(65) && noticker())
			blob_act(40)//almost guaranteed to table-partize it
...
/obj/table/MouseDrop_T(obj/O as obj|mob, mob/user as mob)
	...
	if (ismob(O) && locate(src) in range(1, user))
		...
		if (m == user)
			...
			var/spawntime = rand(50,70)
			if (isAlienLarva(user))
				spawntime = rand(80,100)
			if (isAlienHugger(user))
				spawntime = rand(5,7)
			if (isTrueAlien(user))
				spawntime = rand(10,15)
			...
			if (locate(src) in range(1, user) && locate(src) in range(1, m))
				...
				spawn (10)
					if (locate(src) in range(1, user) && locate(src) in range(1, m))
						m.set_loc(src)
						m.visible_message("<span style=\"color:red\">[user] pushes [m] on to the table!</span>")
						m.weakened += 5
						m.stunned += 5
					else
