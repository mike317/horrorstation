...
/obj/machinery/door/proc/tear_apart(mob/user as mob)
	...
	var/timetopry = 100
	if (isAlien(user))
		timetopry = rand(20,30)
		if (istype(user:mutantrace, /datum/mutantrace/xenomorph/warrior))
			timetopry = rand(7,10)

	if (do_after(user, timetopry) && !(user.stunned > 0 || user.weakened > 0 || user.paralysis > 0 || user.stat != 0 || user.restrained()))
		var/success = 0
		spawn (6)
			if (src)
				if (istype(src, /obj/machinery/door/poddoor) && !isAlien(user))
					boutput(user, "<span style=\"color:red\">The door is too strong for you!</span>")
				else
					src.simple_open_close()
...
/obj/machinery/door/proc/simple_open_close()
	open(1)

/obj/machinery/door/proc/open(var/ignore_all = 0)
	if (!ignore_all)
		if(!density)
			return 1
...
/obj/machinery/door/proc/close(var/ignore_all = 0)
	if (!ignore_all)
		if(density)
			return 1
		if (src.operating)
...
/obj/machinery/door/proc/opened(var/ignore_all = 0)
	if(autoclose)
		sleep(150)
		autoclose(ignore_all)

/obj/machinery/door/proc/closed(var/ignore_all = 0)
	if(mechanics)
		mechanics.fireOutgoing(mechanics.newSignal("doorClosed"))

/obj/machinery/door/proc/autoclose(var/ignore_all = 0)
	if (!ignore_all)
