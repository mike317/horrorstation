...
/obj/machinery/door/airlock/attackby(C as obj, mob/user as mob)
	...
	if ((istype(C, /obj/item/weldingtool) && !( src.operating ) && src.density))
		...
		if (istype(C, /obj/item/crowbar/survivor_crowbar))

		//	src.unpowered_open_close(1)

			user.visible_message("<span style = \"color:red\">[user] starts to pry open the [src].</span>", "<span style = \"color:red\">You start to pry open the [src].</span>")
			spawn(rand(20,30))
				simple_open_close()
				spawn (1)
					if (!src.density)
						user.visible_message("<span style = \"color:red\">[user] pries open the [src].</span>", "<span style = \"color:red\">You pry open the [src].</span>")

		else
			src.unpowered_open_close(0)
	else
		..()
	return

/obj/machinery/door/airlock/proc/unpowered_open_close(var/bypass_power = 0)
	if (!src || !istype(src))
		return
...
/obj/machinery/door/airlock/simple_open_close()
	open(1)
//	spawn (rand(50,75))
//		close(1)

/obj/machinery/door/airlock/open(var/ignoreall = 0)
	if (!ignoreall)
...
/obj/machinery/door/airlock/close(var/ignoreall = 0)
	if (!ignoreall)
		if (src.welded || src.locked || (!src.arePowerSystemsOn()) || (stat & NOPOWER) || src.isWireCut(AIRLOCK_WIRE_OPEN_DOOR))
			return
	if (!ignoreall)
