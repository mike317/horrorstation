...
/obj/machinery/bot
	...
	attack_hand(mob/user)
		if (isAlien(user))
			user.visible_message("<span style = \"color:red\"><B>[user] slashes [src]!</B></span>", "<span style = \"color:blue\"><B>[user] slashes [src]!</B></span>")
			playsound(user.loc, "sound/weapons/slashcut.ogg", 90, 1)
			if (istype(src, /obj/machinery/bot/mulebot))
				alienExplode()//generic explode
			else if (istype(src, /obj/machinery/bot/guardbot))
				explode(0)//guardbot explode
			else
				explode()//this type's explode
			return 1
		return 0
