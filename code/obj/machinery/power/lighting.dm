...
/obj/machinery/light/New()
	...
	spawn(100)
		if (ALIENMODE && prob(98) && noticker())
			status = LIGHT_BROKEN
			update()
/*
	spawn(100)//todo: since this is the only immersion update that actually spawns a new
	//object, move it to somewhere better to account for objects made after the lights
	//right now I'm just adding spawn(100) because I assume all objs will be loaded by then

		if (ALIENMODE && prob(98))
			if (prob(1))
				var/gottabarrel = 0
				for (var/turf/t in range(1, src))
					if (gottabarrel)
						break
					if (istype(t, /turf/simulated/floor) && t.name != "astroturf")
						var/obj = 0
						for (var/obj/o in t)
							if (o.layer > t.layer && !istype(o, /obj/overlay))
								obj = 1
							break
						for (var/obj/reagent_dispensers/fueltank in range(2, t))
							obj = 1
							break

						if (!obj)
							if (!locate(/obj/machinery/door) in range(1, t))
								new/obj/burning_barrel(t)
								gottabarrel = 1
			broken()
*/
