...
/obj/critter/wendigo
	...
	attack_hand(var/mob/user as mob)
		...
		if (user.a_intent == INTENT_HARM)
			src.health -= (rand(1,2) + (user:mutantrace ? user:mutantrace:get_bonus_damage() : 0) ) * src.brutevuln
			on_damaged(src)
			for(var/mob/O in viewers(src, null))
				if (!isAlien(user))
					O.show_message("<span style=\"color:red\"><b>[user]</b> punches [src]!</span>", 1)
				else
					O.show_message("<span style=\"color:red\"><b>[user]</b> slashes [src]!</span>", 1)

			if (!isAlien(user))
				playsound(src.loc, "punch", 50, 1)
			else
				playsound(src.loc, "sound/weapons/slashcut.ogg", 50, 1)
