...
/obj/item/organ
	...
	proc/xeno_devour(var/mob/living/carbon/human/H)
		if (isAlienHugger(H))
			return 0

		visible_message("<span style = \"color:red\"><b>[H] starts to devour [src]!</span></b>")
		sleep(rand(3,7))
		devour_health -= rand(10,20)
		if (devour_health <= 0)
			visible_message("<span style = \"color:red\"><b>[H] finishes devouring [src]!</span></b>")
			qdel(src)

		if (isAlien(H))
			var/datum/mutantrace/xenomorph/x = H.mutantrace
