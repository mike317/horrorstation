/obj/item/raw_material/
	...
	var/ghetto_refine_stage = STAGE_START
	var/end_at_stage = STAGE_FINAL

	//the idea with the consts below is to find a material that can get
	//extremely hot without falling apart, as that weapon will be the best
	//against xenos. Even better if it's flammable
	var/const/smelt_temp = 100
	var/const/smelt_temp2 = 130
	var/const/melt_temp = 160
	var/const/fall_apart_temp = 200
	var/const/flammable = 0

	//time it takes to manipulate this material by default
	var/const/manipulation_time = 100
...
/obj/item/raw_material/wood
	name = "wood"
	desc = "A chunk of wood. Not bad for making shitty weapons or armor."
	icon_state = "rock1"//change this
	force = 3
	throwforce = 5
	scoopable = 0
...
/obj/item/raw_material/shard
	...
	HasEntered(AM as mob|obj)
		if(ismob(AM))
			var/mob/M = AM
			if (ishuman(M) && M:mutantrace && istype(M:mutantrace, /datum/mutantrace/xenomorph))
				return
			if(ishuman(M))
