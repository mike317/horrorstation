...
/obj/decal/cleanable/blood/gibs
	...
	attack_hand(var/mob/user as mob)
		if (!isAlien(user))
			...
			if (!isAlienHugger(user))
				visible_message("<span style = \"color:red\"><b>[user] devours a bit of [src]!</span></b>")
				sleep(rand(3,7))
				devour_health -= rand(10,20)
				if (devour_health <= 0)
					visible_message("<span style = \"color:red\"><b>[user] finishes devouring [src]!</span></b>")
					qdel(src)

				if (isAlien(user))
					var/datum/mutantrace/xenomorph/x = user:mutantrace
					x.maxPlasma += rand(2,3)

				playsound(user.loc, "sound/effects/gib.ogg", rand(50,75), 1)
				return 1
