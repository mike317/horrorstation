/obj/item
	...
	throwforce = 4				//DAMAGE_CUT, DAMAGE_STAB, now affect ghetto material refining, cut more than stab
	var/r_speed = 1.0
...
/obj/item/proc/Eat(var/mob/M as mob, var/mob/user)
	...
	if (M == user)
		...
		if (ishuman(M))
			...
			H.will_starve_in_deciseconds += rand(250,350)
			if (H.will_starve_in_deciseconds > 6000)
				H.will_starve_in_deciseconds = 6000
...
/obj/item/attack_hand(mob/user as mob)
	if (user.bioHolder.HasEffect("xeno") && !user.find_in_hand(src))
		if (ishuman(user))
			return user:mutantrace:custom_attack(src)
