...
/mob/living/carbon/proc/grab_self()
	...
	if (isAlien(src))
		return
	if (src.buckled && istype(src.buckled, /obj/xeno/hive/nest) || src.buckled && istype(src.buckled, /obj/stool/bed))
		return
...
/mob/proc/calculate_disarm_attack(var/mob/target, var/obj/item/affecting, var/base_damage_low = 0, var/base_damage_high = 0, var/extra_damage = 0)
	...
	if (ishuman(target))
		...
		if (ishuman(src))
			...
			if (isAlien(target))
				msgs.base_attack_message = "<span style=\"color:red\"><B>[src] shoves [target], but it has no effect!</span></b>"
...
//	if (target:mutantrace)
	//	damage *= target:mutantrace:brutevuln //I think this is handled elsewhere
