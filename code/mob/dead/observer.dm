// Observer
var/aghosted_xenos[0]//meme list designed to prevent xeno gamemode from instantly
//ending if the one xeno admin ghosts
var/aghosted_humans[0]
...
/mob/proc/ghostize()
	var/was_alien = isAlien(src)
	var/was_human = ishuman(src)
	if (isAlien(src))
		was_human = 0
	...
	if(src.key || src.client)
		...
		if (src.client && src.client.holder && src.stat != 2)
			O.stat = 0
			if (was_alien)
				aghosted_xenos += src.client
				spawn (6000)//sleep ten (byond) minutes
					aghosted_xenos -= src.client//if they're still a fukken ghost
					//enough is enough, end the goddamn round
					if (aghosted_humans.Find(src.client))
						aghosted_humans -= src.client
					if (aghosted_xenos.Find(src.client))
						aghosted_xenos -= src.client

			else if (was_human)
				aghosted_humans += src.client
				spawn (6000)//sleep ten (byond) minutes
					aghosted_humans -= src.client//if they're still a fukken ghost
					//enough is enough, end the goddamn round
					if (aghosted_humans.Find(src.client))
						aghosted_humans -= src.client
					if (aghosted_xenos.Find(src.client))
