mob/new_player
	...
	verb/new_player_panel()
		...
		if(client)
			if (global.ticker && global.ticker.round_elapsed_ticks > 100 && global.ticker.mode.late_crew_join == 0)
				winset(src, "joinmenu.button_ready", "is-disabled=true;is-visible=true")
			else
...
*/
	...
	proc/AttemptLateSpawn(var/datum/job/JOB)
		...
		if (JOB && IsJobAvailable(JOB))
			...
			var/miscreant = 0
#ifdef MISCREANTS
			if (ticker && character.mind && JOB.allow_traitors != 0 && prob(10) && !ALIENMODE)
			...
			if (ticker && character.mind && !miscreant && !ALIENMODE)
...
// This fxn creates positions for assistants based on existing positions. This could be more elgant.
	...
	proc/create_character(var/datum/job/J, var/allow_late_antagonist = 0)
		...
		return new_character
				...
				//actual making into xeno happens in gameticker.dm, postsetup() (xenomorph.dm)
	Move()
