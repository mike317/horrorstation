...
/mob/proc/xenomorphize(var/caste, var/roundstart = 0)
	if (src.transforming)
		return
	if (iswizard(src))
		src.visible_message("<span style=\"color:red\"><b>[src] magically resists being transformed!</b></span>")
	...
	if (ishuman(src))
		...
		for (var/mob/living/carbon/human/H in range(ALIENVISION_RANGE, src))
			if (isAlien(H))
				hmob.human_infrared_image(H, 1)

	if (roundstart)
		src.make_true_xenomorph(0, 0, caste)
	else
		src.make_true_xenomorph(0, 1, caste)
//	spawn (7)
	//	H.xSetMobEvolution(caste, 0, 1)

//	src.unequip_all()

//	src.bioHolder.AddEffect("xeno")
	return
