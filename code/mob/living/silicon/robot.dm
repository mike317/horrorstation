...
//3 Modules can be activated at any one time.
				...
				switch(user.a_intent)
					...
					if(INTENT_HARM)
						if (!isAlien(user))//Dumbo
							...
							user.visible_message("<span style=\"color:red\"><B>[user] slashes [src]!</span>", "<span style=\"color:red\"><B>You slash [src]!</B></span>")
							random_brute_damage(src, user:mutantrace:get_bonus_damage())
						//	random_brute_damage(user, rand(2,5))
						//	if(prob(10)) user.show_text("Your hand hurts...", "red")
	...
	Topic(href, href_list)
		..()
		if (usr.restrained() || usr.lying || usr.stat || get_dist(usr, src) > 1)
			return 1
