...
/obj/human_light
	...
	icon_state = "infraoverlay"
	var/datum/light/infrared_light
	invisibility = 2
	...
	proc/update_icon()
		if (holder && holder.lying)
			icon_state = "infraoverlay_lying"
			...
			if (holder)
				switch (holder.dir)
					...
					if (SOUTH)
						icon_state  = "infraoverlay"
					if (EAST)
						icon_state  = "infraoverlay_E"
					if (WEST)
						icon_state  = "infraoverlay_W"

	New()
	//	infrared_light = new/datum/light/point
	//	infrared_light.attach(src)
	//	infrared_light.set_color(0.66, 0.1, 0.1)
	//	infrared_light.enable()
		..()
	Del()
		//infrared_light.disable()
		..()
	...
	attack_hand(mob/user as mob)
		...
		if (H && !H.mutantrace)
			if (!H.r_hand && !H.l_hand)
				return holder.attack_hand(H)
			else
				if (H.hand == 0 && H.r_hand || H.hand == 1 && H.l_hand)
					var/chand = H.hand == 0 ? H.r_hand : H.l_hand
					return holder.attackby(chand, H)//apparently I can't use attackby for objects?
				else
					return holder.attack_hand(H)

		else if (H && H.mutantrace)
			if (!H.r_hand && !H.l_hand)
				return H.mutantrace.custom_attack(holder)
			else
				if (H.hand == 0 && H.r_hand || H.hand == 1 && H.l_hand)
					var/chand = H.hand == 0 ? H.r_hand : H.l_hand
					return holder.attackby(chand, H)//apparently I can't use attackby for objects?
...
//	var/obj/item/device/radio/w_radio = null
	...
	var/datum/light/xeno_light//sentinels emit light right now
	var/datum/light/infrared_light
	...
	var/static/image/nested_image = image('icons/tg-goon-xenos/xeno.dmi')
...
/*
	var/obj/human_light/afukkenlight = new/obj/human_light(src.loc)
	src.my_light_obj = afukkenlight
	src.my_light_obj.holder = src
	*/

	for (var/mob/living/carbon/human/H in range(ALIENVISION_RANGE, src))
		if (isAlien(H))
			human_infrared_image(H)
...
/mob/living/carbon/human/proc/is_xenomorph()
	return get_ability_holder(/datum/abilityHolder/xenomorph)
...
//	if (locate(/obj/xeno/hive/weeds) in src.loc && isAlien(src))
//		if (prob(99))
//			return
...
/mob/living/carbon/human/emote(var/act, var/voluntary = 0)
	for (var/v in prohibited_strings)
		if (findtext(act, v))//I somehow doubt this is possibly but doing it anyway
			return
	...
	for (var/datum/ailment_data/data in src.ailments)
		if (istype(data.master, /datum/ailment/parasite/alien_larva))
			if (lying)//prevents emote spam when the only thing keeping a mob
			//alive is a larva inside them
				if (weakened > 1)
					return

	if (buckled && istype(buckled, /obj/xeno/hive/nest))
		if (weakened > 1)
			return//ditto
	...
	if (!message)
		...
		if (src.mutantrace && src.mutantrace.emotes_allowed.Find(lowertext(act)) == 0)
			if (src.mutantrace.emotes_allowed.len > 0)
				goto showmessage
					...
					if ((!istype(src.loc, /turf/space)) && (!src.on_chair))
						if (!src.lying)
							...
							for (var/mob/living/M in view(1, null))
								...
								if (M == src)
									continue
								if (isAlien(M) && M:mutantrace:bigXeno)//suplexing queens was hilarious but too memey for this
								//server
									continue
								if (istype(usr.equipped(), /obj/item/grab))
									if (G.state >= 1)
										...
										if (iswrestler(src))
											if (prob(50))
												...
												if (!isAlien(G.affecting))
													G.affecting.stunned += 4
													G.affecting.weakened += 4
												else
													G.affecting.stunned += 2
...
*/
	...
	if (src.bleeding && src.stat != 2)
		...
		else//keeping this the same for now
			switch (src.bleeding)
				if (1 to 2)
					. += "<br><span style=\"color:red\">[src.name] is bleeding a little bit.</span>"
				if (3 to 5)
	...
	if (!isvampire(src) && !isAlien(src) && (src.blood_volume < 500)) // Added a check for vampires (Convair880).
...
/mob/living/carbon/human/movement_delay()
	...
	if (src.reagents && src.reagents.has_reagent("methamphetamine"))
		if (!src.bioHolder || !src.bioHolder.HasEffect("revenant"))
			return -1
	if (src.nodamage || isAlien(src))//will probably fix the fucking stupid slowdown
	//atmos bug for aliens
		return -1
...
/mob/living/carbon/human/Stat()
	...
	if (!isAlien(src))
		...
		if (src.mutantrace:pheromones)
			...
			if (istype(global.ticker.mode, /datum/game_mode/xeno))

				if (global.ticker.mode:roundstart_aliens == 1)
					expediated_evolution = 1

				if (src:mutantrace:maxEvoProgress == initial(src:mutantrace:maxEvoProgress))

					if (isAlienDrone(src))
						if (global.ticker.mode:isSentinel() == 0 && global.ticker.mode:was_alien_sentinel)
							expediated_evolution = ticker.mode:was_alien_sentinel
						if (global.ticker.mode:was_alien_sentinel == 0 && global.ticker.mode:was_alien_warrior == 0 && global.ticker.mode:was_alien_praetorian == 0)
							if (global.ticker.mode:was_alien_queen && global.ticker.mode:isQueen() == 0)
								expediated_evolution = ticker.mode:was_alien_queen
					...
					if (isAlienSentinel(src))
						...
						if (global.ticker.mode:isWarrior() == 0 && global.ticker.mode:was_alien_warrior)
							no_warrior = 1

						if (global.ticker.mode:isPraetorian() == 0 && global.ticker.mode:was_alien_praetorian)
							no_prae = 1

						if (no_warrior && no_prae)
							expediated_evolution = ticker.mode:was_alien_warrior + ticker.mode:was_alien_praetorian
								//sometimes this will result in EXTREMELY fast evolution. Of course, it's very hard for warriors/praes to die

					if (isAlienPraetorian(src))

						if (global.ticker.mode:isQueen() == 0 && global.ticker.mode:was_alien_queen)
							expediated_evolution = ticker.mode:was_alien_queen

					if (expediated_evolution)
...
/mob/living/carbon/human/Move(a, b, flag)
	...
	if ((t7 && (src.pulling && ((get_dist(src, src.pulling) <= 1 || src.pulling.loc == src.loc) && (src.client && src.client.moving)))))
		...
		if (ishuman(src.pulling))
			var/mob/living/carbon/human/hmob = src.pulling
			for (var/mob/living/carbon/human/H in range(ALIENVISION_RANGE, hmob))
				if (isAlien(H))
					hmob.human_infrared_image(H)
	else
		src.pulling = null
		if (xenoHud)
			xenoHud.update_pulling()
...
/mob/living/carbon/human/attack_hand(mob/living/carbon/human/M as mob)
	...
	if (isAlien(M))
		if (ishuman(src))
			for (var/datum/ailment_data/data in src.ailments)
				if (istype(data.master, /datum/ailment/parasite/alien_larva))
					if (M.a_intent == "harm")
						boutput(M, "You cannot attack an infected host.")
						return 0
	...
	switch(M.a_intent)
		...
		if (INTENT_GRAB)//insert alien grabbing here plz
		...
		if (INTENT_HARM)
			if (isAlien(M))
				return M.xmelee_attack_normal(src, M.mutantrace.get_bonus_damage(), 0, DAMAGE_CRUSH)
			if (M.is_mentally_dominated_by(src))
				boutput(M, "<span style=\"color:red\">You cannot harm your master!</span>")
				return
...
/mob/living/carbon/human/restrained()
	if (src.handcuffed || src.buckled && istype(src.buckled, /obj/xeno/hive/nest))
		return 1
	...
	if ((src.item && !( L.Find(src.place) )))
		...
		if (src.place == "syringe")
			...
			if (src.place == "pill")
				...
				if (src.place == "fuel")
					src.target.visible_message("<span style=\"color:red\">[src.source] is trying to force [src.target] to eat the [src.item]!</span>")
				else																													//this was src.item:content, but that caused compile errors?
					if (src.place == "drink")
...
//		blood_volume = max(blood_volume, 0)
	...
	proc/handle_temperature_damage(body_part, exposed_temperature, exposed_intensity)
		...
		if (exposed_temperature < src.base_body_temp && isAlien(src))//no cold damage for aliens
			return

		var/discomfort = min(abs(exposed_temperature - bodytemperature)*(exposed_intensity)/2000000, 1)

		if (isAlien(src))
			ambient_xeno_update(0,0,1,0,0,1)
...
//				blood = reagents.get_reagent("blood")
	...
	proc/handle_regular_status_updates(datum/controller/process/mobs/parent)
		...
		if (istype(src.wear_mask, /obj/item/xeno/facehugger))
			src.blinded = 1
...
////Reagents
	...
	proc/handle_regular_hud_updates()
		...
		if (xenoHud)
			xenoHud.update_health_indicator()
		else
		...
		if (xenoHud)
			xenoHud.update_blood_indicator()
	...
	proc/check_if_buckled()
		if (src.buckled)
			if (src.buckled.loc != src.loc)
				src.buckled = null
				return
			if (istype(src.buckled, /obj/stool/bed) || istype(src.buckled, /obj/machinery/conveyor) || istype(src.buckled, /obj/xeno/hive/nest))
				src.lying = 1
			if (src.lying)
				src.drop_item()
			src.density = 1
...
/mob/living/carbon/human/say(var/message)
	...
	if (!dd_hasprefix(message, "*") && isAlien(src))
		playsound(src.loc, src.mutantrace:sound_alienspeak1, 75, 1)
...
/mob/living/carbon/human/whisper(message as text)
	...
	for (var/v in prohibited_strings)
		if (findtext(message_check, v))
			logTheThing("say", src, null, "attempted to access Topic() through whisper()")
			return
...
/mob/living/carbon/human/put_in_hand(obj/item/I, hand)
	...
	if (isAlien(src))
		var/obj/xeno/x = I
		if (!istype(x, /obj/item/xeno) && !istype(I, /obj/item/grab))
			return 0
	...
	if (isnull(hand))
		...
		if (hand)
			if (!src.l_hand)
				...
				src.update_inhands()
				if (xenoHud)
					xenoHud.add_object(I, HUD_LAYER+2, ui_lhand)
				else
			...
			if (!src.r_hand)
				...
				src.update_inhands()
				if (xenoHud)
					xenoHud.add_object(I, HUD_LAYER+2, ui_rhand)
				else
...
/mob/living/carbon/human/proc/force_equip(obj/item/I, slot)
	...
	if (isAlien(src))
		if (slot != slot_back && slot != slot_l_hand && slot != slot_r_hand)
			return 0
	...
	switch(slot)
		if (slot_back)
			if (!src.back)
				...
				if (xenoHud)
					xenoHud.add_object(I, HUD_LAYER+2, ui_back)
				else
		...
		if (slot_wear_mask)
			if (!src.wear_mask && src.organHolder && src.organHolder.head)
				src.wear_mask = I
				if (xenoHud)
					xenoHud.add_other_object(I, ui_mask)
				else
		...
		if (slot_belt)
			if (!src.belt)
				src.belt = I
				if (xenoHud)
					xenoHud.add_object(I, HUD_LAYER+2, ui_belt)
				else
		...
		if (slot_wear_id)
			if (!src.wear_id)
				src.wear_id = I
				if (xenoHud)
					xenoHud.add_other_object(I, ui_id)
				else
		...
		if (slot_ears)
			if (!src.ears && src.organHolder && src.organHolder.head)
				src.ears = I
				if (xenoHud)
					xenoHud.add_other_object(I, ui_ears)
				else
...
/mob/living/carbon/human/full_heal()
	...
	if (isAlien(src))
		var/datum/mutantrace/xenomorph/x = src.mutantrace
		x.plasma = x.maxPlasma
...
/mob/living/carbon/human/infected(var/datum/pathogen/P)
	if (src.stat == 2)
		return
	if (ischangeling(src) || isvampire(src) || isAlien(src)) // Vampires were missing here. They're immune to old-style diseases too (Convair880).
		return 0
...
/mob/living/carbon/human/proc/resist()
	...
	if (src.buckled && istype(src.buckled, /obj/xeno/hive/nest))
		for (var/mob/O in AIviewers(src))
			O.show_message("<span style=\"color:red\"><B>[src] attempts to break free from the nest!</span></B>")
			if (prob(5))
				src.anchored = 0
				src.buckled = null
				src.update_clothing()
				spawn (3)
					O.show_message("<span style=\"color:red\"><B>[src] breaks free from the nest!</span></b>")
				return 1
		...
		if (src.wear_mask && istype(src.wear_mask, /obj/item/clothing/mask))
			if (prob(src.wear_mask.armor_value_melee * 10 > 70 ? 70 : src.wear_mask.armor_value_melee))
				...
				if (prob(src.wear_mask.armor_value_melee * 1))//I mean it's just a mask
					src.visible_message("<span style = \"color:red\"><b>The facehugger is smashed by [src]'s mask!</span></b>")
...
/mob/living/carbon/human/canRideMailchutes()
	if (isAlienHugger(src) || isAlienLarva(src) || isAlienDrone(src) || isAlienSentinel(src) || isAlienWarrior(src) )
		return 1
	if (mutantrace && istype(mutantrace, /datum/mutantrace/xenomorph))
		return 0
