...
/mob/living/carbon/human/update_clothing(var/loop_blocker)
	...
	if (src.buckled)
		if (istype(src.buckled, /obj/stool/bed) || istype(src.buckled, /obj/xeno/hive/nest))
			src.lying = 1
		else
			src.lying = 0
	...
	if (!src.mutantrace || !src.mutantrace.override_damage_detail)
	...
	if (src.blood_DNA && !src.gloves)
		...
		if (src.limbs && src.limbs.l_arm && !istype(src.limbs.l_arm, /obj/item/parts/robot_parts))
			...
			if (!src.mutantrace || !src.mutantrace.override_blood)
...
//		src.wear_id.screen_loc = ui_id
	...
	if (src.buckled && istype(src.buckled, /obj/xeno/hive/nest))
		src.pulling = null
		nested_image.icon_state = "nestoverlay"
		nested_image.layer = MOB_HANDCUFF_LAYER
		UpdateOverlays(nested_image, "handcuffs")
	else
		UpdateOverlays(null, "handcuffs")
...
/mob/living/carbon/human/proc/update_infrared_light()
	...
	if (!isAlien(src))
		...
		if (isliving(src))
			...
			infrared_light.set_brightness(2.0)
...
/mob/living/carbon/human/update_burning_icon(var/old_burning)
	...
	if (!mutantrace || !mutantrace.override_fireover)
