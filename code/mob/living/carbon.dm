...
/mob/living/carbon/add_stamina(var/x as num)
	...
	if (isAlien(src) && locate(/obj/xeno/hive/weeds) in src.loc)
		stamina = min(stamina_max, stamina + (x*2))
	else
		stamina = min(stamina_max, stamina + x)

	if (ishuman(src))
		var/mob/living/carbon/human/H = src
		H.ambient_xeno_update(0,0,0,1,0,1)
...
/mob/living/carbon/remove_stamina(var/x)
	...
	if (src:mutantrace)
		x /= src:mutantrace:stamina_divider
...
/mob/living/carbon/take_brain_damage(var/amount)
	...
	if (src.brainloss >= 120 && !isAlien(src))
