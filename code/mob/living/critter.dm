	...
	attack_hand(var/mob/living/M)
		switch (M.a_intent)
			...
			if (INTENT_HARM)
				...
				var/bonus_damage = 0
				if (ishuman(M) && M:mutantrace)
					bonus_damage = M:mutantrace.get_bonus_damage()

				src.TakeDamage(M.zone_sel.selecting, rand(1,3) + bonus_damage, 0)

				if (ishuman(M) && M:mutantrace && istype(M:mutantrace, /datum/mutantrace/xenomorph))
					playsound_local(src, "sound/weapons/slashcut.ogg", 50, 0)
				else
					playsound_local(src, "punch", 50, 0)

				visible_message("<b><span style='color:red'>[M] [ishuman(M) && M:mutantrace && istype(M:mutantrace, /datum/mutantrace/xenomorph) ? "slashes" : "punches"] [src]!</span></b>")
			if (INTENT_GRAB)
