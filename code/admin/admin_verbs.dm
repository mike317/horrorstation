...
/client/proc/simple_ban,\
/client/proc/simple_unban,\
...
/client/proc/simple_ban()
	...
	if (src.holder.level < LEVEL_SA)
		return
	var/cont = input("Continue?") in list ("Yes", "No")
	if (cont == "Yes")
		var/selection[0]
		for (var/client/c in clients)
			selection += c
		...
		var/client/who_to_ban = input("Ban what client?") in selection
		var/ban_reason = input("Ban them for what reason?") as text
		var/hours = input("For how many hours?(defaults to 1)") as num

		if (!istype(who_to_ban))
			for (var/client/c in clients)
				if ("[c]" == who_to_ban)
					who_to_ban = c

		if (who_to_ban && istype(who_to_ban) && who_to_ban != "Nobody")
			who_to_ban.saddban(hours, ban_reason, src)
...
/client/proc/simple_unban()
	...
	if (cont == "Yes")
		var/their_address = input("Unban what address?") as text
		var/their_ckey = input("Unban what ckey?") as text
		var/their_compid = input("Unban what computer id?") as text

		var/unban_reason = input("Unban them for what reason?") as text

		sunban(their_address, their_ckey, their_compid, unban_reason, src)
	//end simple banning
