	...
	//		var/list/alljobs = get_all_jobs()
		...
		if("transform")
			if(( src.level >= LEVEL_PA ) || ((src.level >= LEVEL_SA) ))
				...
				var/which = input("Transform them into what?","Transform") as null|anything in list("Monkey","Cyborg","Lizardman","Squidman","Martian","Skeleton","Flashman","Ghostdrone", "Xenomorph Drone", "Xenomorph Sentinel", "Xenomorph Warrior", "Xenomorph Praetorian", "Xenomorph Queen", "Xenomorph Empress", "Facehugger")
				if (!which)
					return
				. = 0
				switch(which)
					...
					if ("Xenomorph Drone")
						H.xenomorphize("Drone")

					if ("Xenomorph Sentinel")
						H.xenomorphize("Sentinel")

					if ("Xenomorph Warrior")
						H.xenomorphize("Warrior")

					if ("Xenomorph Praetorian")
						H.xenomorphize("Praetorian")

					if ("Xenomorph Queen")
						H.xenomorphize("Queen")

					if ("Xenomorph Empress")
						H.xenomorphize("Empress")

					if ("Facehugger")
						H.xenomorphize("Facehugger")
		...
		if ("create_object")
			if (src.level >= LEVEL_PA)
				create_object(usr)
				logTheThing("admin", usr, null, "opened the create_object panel")
				logTheThing("diary", usr, null, "opened the create_object panel", "admin")
				message_admins("[key_name(usr)] is opening the create_object panel")
			else
		...
		if ("create_turf")
			if (src.level >= LEVEL_PA)
				create_turf(usr)
				logTheThing("admin", usr, null, "opened the create_turf panel")
				logTheThing("diary", usr, null, "opened the create_turf panel", "admin")
				message_admins("[key_name(usr)] is opening the create_turf panel")
			else
		...
		if ("create_mob")
			if (src.level >= LEVEL_PA) // Moved from SG to PA. They can do this through build mode anyway (Convair880).
				create_mob(usr)
				logTheThing("admin", usr, null, "opened the create_mob panel")
				logTheThing("diary", usr, null, "opened the create_mob panel", "admin")
				message_admins("[key_name(usr)] is opening the create_mob panel")
			else
			...
			if (src.level >= LEVEL_SA)
				switch(href_list["type"])
					...
					if("transform_one")
						...
						var/which = input("Transform them into what?","Transform") as null|anything in list("Monkey","Cyborg","Lizardman","Squidman","Martian","Skeleton","Flashman","Xenomorph Drone", "Xenomorph Sentinel", "Xenomorph Warrior", "Xenomorph Praetorian", "Xenomorph Queen", "Xenomorph Empress")
						if (!which)
							return
						switch(which)
					...
					if("transform_all")
						var/which = input("Transform everyone into what?","Transform") as null|anything in list("Monkey","Cyborg","Lizardman","Squidman","Martian","Skeleton","Flashman", "Xenomorph Drone", "Xenomorph Sentinel", "Xenomorph Warrior", "Xenomorph Praetorian", "Xenomorph Queen", "Xenomorph Empress")
	...
	*/
		...
		if ("humanize")
			if (src.level >= LEVEL_PA) // Moved from SG to PA (Convair880).
				...
				M.humanize(0,0,1)
				*/
		//		var/mob/living/carbon/human/newM = /*usr.client.cmd_admin_humanize(M)*/ M.humanize(0,0,1)
	//			href_list["target"] = "\ref[newM]"
...
/datum/admins/proc/announce()
	...
	var/alienmsg = input("Announce this to aliens only?") in list("Yes", "No")
	...
	if (message)
		...
		for (var/mob/m in world)
			if (alienmsg == "Yes" && !isAlien(m))
				if (!m.client || !m.client.holder)
					continue

			boutput(m, "<span style=\"color:blue\"><b>[admin_key(usr.client, 1)] Announces[alienmsg == "Yes" ? " to aliens" : ""]:</b><br>&emsp; [message]</span>")

		logTheThing("admin", usr, null, ": [alienmsg == "Yes" ? "alien message" : ""][message]")
		logTheThing("diary", usr, null, ": [alienmsg == "Yes" ? "alien message" : ""][message]", "admin")
