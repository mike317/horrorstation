...
/mob/living/apply_flash(var/animation_duration = 30, var/weak = 8, var/stun = 0, var/misstep = 0, var/eyes_blurry = 0, var/eyes_damage = 0, var/eye_tempblind = 0, var/burn = 0, var/uncloak_prob = 50)
	if (!src || !isliving(src) || isintangible(src) || isAlien(src) || istype(src, /mob/living/object))
		return
...
/mob/proc/smash_through_turf(var/turf/target, var/list/can_smash)
	if (!src || !ismob(src) || !target || !isturf(target))
		return 0

	if (!islist(can_smash) || !can_smash.len)
		return 0

	var/bonus_smash_chance = 0
	if (ishuman(src) && src:mutantrace && istype(src:mutantrace, /datum/mutantrace/xenomorph))
		bonus_smash_chance = src:mutantrace:get_turf_destroy_bonus_chance()

	for (var/S in can_smash)
		if (S == "wall" && istype(target, /turf/simulated/wall/supernorn) && target:mod != "R")
			var/turf/simulated/wall/T = target
			T.health -= rand(7,10) + bonus_smash_chance
			src.visible_message("<span style=\"color:red\">[src] smashes against the wall.</span>", "<span style=\"color:blue\">You smash through the wall.</span>")
			if (T.health <= 0)
				src.visible_message("<span style=\"color:red\">[src] smashes through the wall.</span>", "<span style=\"color:blue\">You smash through the wall.</span>")
				T.dismantle_wall(1)
			return 1
	return 0
...
/mob/proc/smash_through(var/obj/target, var/list/can_smash)
	...
	var/bonus_smash_chance = 0
	if (ishuman(src) && src:mutantrace && istype(src:mutantrace, /datum/mutantrace/xenomorph))
		bonus_smash_chance = src:mutantrace:get_object_destroy_bonus_chance()

	for (var/S in can_smash)
		if (S == "shrub" && istype(target, /obj/shrub))
			var/obj/shrub = target
			qdel(shrub)
			return 1
		if (S == "reagentdispenser" && istype(target, /obj/reagent_dispensers))
			var/obj/reagentdispenser = target
			qdel(reagentdispenser)
			return 1
		...
		if (S == "morgue" && istype(target, /obj/morgue))
			...
			m.visible_message("<span style=\"color:red\"><b>[src] smashes [m] into pieces!</span></b>")
			m.ex_act(1.0)
			return 1

		if (S == "crematorium" && istype(target, /obj/crematorium))
			var/obj/crematorium/c = target
			c.visible_message("<span style=\"color:red\"><b>[src] smashes [c] into pieces!</span></b>")
			c.ex_act(1.0)
			return 1

		if (S == "displaycase" && istype(target, /obj/displaycase))
			var/obj/displaycase/dc = target
			dc.visible_message("<span style=\"color:red\"><b>[src] violently slashes [dc]!</span></b>")
			dc.ex_act(1.0)
			return 1

		if (S == "clothing" && istype(target, /obj/item/clothing))
			src.visible_message("<span style=\"color:red\">[src] rips the [target] to shreds!</span>", "<span style=\"color:blue\">You rip the [target] to shreds.</span>")
			qdel(target)
			return 1

		if (S == "plastic flaps" && istype(target, /obj/plasticflaps))
			src.visible_message("<span style=\"color:red\">[src] rips the plastic flaps to shreds!</span>", "<span style=\"color:blue\">You rip the plastic flaps to shreds.</span>")
			qdel(target)
			return 1
		...
		if (S == "window" && istype(target, /obj/window))
			...
			if (prob(15 + bonus_smash_chance))
				W.ex_act(1.0)
				src.visible_message("<span style=\"color:red\">[src] smashes the window into pieces.</span>", "<span style=\"color:blue\">You smash the window into pieces.</span>")
			else
				src.visible_message("<span style=\"color:red\">[src] smashes against the window.</span>", "<span style=\"color:blue\">You smash against the window.</span>")
		...
		//		playsound(G.loc, "sound/effects/grillehit.ogg", 80, 1)
				if (prob(30 + bonus_smash_chance))
					G.damage_slashing(15)
				return 1

		if (S == "fitness" && istype(target, /obj/fitness))
			var/obj/fitness/f = target
			src.visible_message("<span style=\"color:red\"><b>[src]</b> violently slashes [f]!</span>")
			qdel(f)
			return 1
		...
		if (S == "stool" && istype(target, /obj/stool))
			var/obj/stool/x = target
		...
		if (S == "rack" && istype(target, /obj/rack))
			var/obj/rack/x = target
			if (prob(25 + bonus_smash_chance))
				src.visible_message("<span style=\"color:red\">[src] smashes [x] into pieces.</span>", "<span style=\"color:blue\">You smash [x] into pieces.</span>")
				x.ex_act(1.0)
			else
		...
		if (S == "structure" && istype(target, /obj/structure))
			var/obj/structure/x = target
			if (prob(25 + bonus_smash_chance))
				src.visible_message("<span style=\"color:red\">[src] smashes [x] into pieces.</span>", "<span style=\"color:blue\">You smash [x] into pieces.</span>")
				x.ex_act(1.0)
			else
		...
		if (S == "machinery" && istype(target, /obj/machinery))
			var/obj/machinery/x = target

			if (istype(target, /obj/machinery/camera))
				var/obj/machinery/camera/c = target
				if (prob(50 + bonus_smash_chance))
					src.visible_message("<span style=\"color:red\">[src] breaks [c].</span>", "<span style=\"color:blue\">You break [c].</span>")
					qdel(c)
				else
					src.visible_message("<span style=\"color:red\">[src] smashes [c].</span>", "<span style=\"color:blue\">You smash [c].</span>")
				return 1

			if (istype(target, /obj/machinery/light) && !istype(target, /obj/machinery/light/small))
				var/obj/machinery/light/l = target
				if (prob(50 + bonus_smash_chance))
					src.visible_message("<span style=\"color:red\">[src] breaks [l].</span>", "<span style=\"color:blue\">You break [l].</span>")
					qdel(l)
				else
					src.visible_message("<span style=\"color:red\">[src] smashes [l].</span>", "<span style=\"color:blue\">You smash [l].</span>")
				return 1

			else if (istype(target, /obj/machinery/light/small))
				var/obj/machinery/light/small/l = target
				if (prob(50 + bonus_smash_chance))
					src.visible_message("<span style=\"color:red\">[src] breaks [l].</span>", "<span style=\"color:blue\">You break [l].</span>")
					qdel(l)
				else
			...
			if (istype(target, /obj/machinery/firealarm))
				var/obj/machinery/firealarm/a = target
				if (prob(50 + bonus_smash_chance))
					src.visible_message("<span style=\"color:red\">[src] breaks [a].</span>", "<span style=\"color:blue\">You break [a].</span>")
					qdel(a)
				else
					src.visible_message("<span style=\"color:red\">[src] smashes [a].</span>", "<span style=\"color:blue\">You smash [a].</span>")
				return 1
		...
		if (S == "all unanchored dense objects" && istype(target, /obj) && target.density && !target.anchored && !istype(target, /obj/decal))
			if (istype(target, /obj/structure))
				return 0
			var/obj/o = target
			robogibs(o.loc)
			src.visible_message("<span style=\"color:red\">[src] smashes [o] into pieces.</span>", "<span style=\"color:blue\">You smash [o] into pieces.</span>")
			qdel(o)
			return 1
