...
/obj/item/electronics/battery/attackby(obj/item/W as obj, mob/user as mob)
	if (ishuman(user))
		if (istype(W, /obj/item/cable_coil))
			...
			if (ishuman(user))
				...
				H.equip_if_possible(ba)

/obj/item/electronics/batteryassembly
	name = "Battery-Coil Assembly"
	icon_state = "battcoil"
	module_research = list("electronics" = 2)
