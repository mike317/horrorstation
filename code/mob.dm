	...
	if (lying)
		...
		switch (dir)
			...
			if (SOUTH)
				state  = "infraoverlay"
			if (EAST)
				state  = "infraoverlay_E"
			if (WEST)
				state  = "infraoverlay_W"

	for (var/image/i in recipient.client.images)
		if (!locate(/mob) in i.loc && findtext(i.icon_state, "infraoverlay"))
			recipient.client.images -= i

	if (!remove_image)

		var/image/i = image('icons/mob/mob.dmi', src.loc, state, layer+1)
		recipient.client.images += i
...
/mob/Bump(atom/movable/AM as mob|obj, yes)
	...
	if (ismob(AM))
		...
		if (isTrueAlien(tmob) && !isAlienLarva(tmob))
			size_of_mob = 3
		if (isAlienPraetorian(tmob))
			size_of_mob = 4
		if (isAlienQueen(tmob) || isAlienEmpress(tmob))
			size_of_mob = 5
					...
					if (ishuman(src))
						...
						for (var/mob/living/carbon/human/H in range(ALIENVISION_RANGE, hmob))
							if (isAlien(H))
								hmob.human_infrared_image(H)
					if (ishuman(victim))
						var/mob/living/carbon/human/hmob = victim
						for (var/mob/living/carbon/human/H in range(ALIENVISION_RANGE, hmob))
							if (isAlien(H))
								hmob.human_infrared_image(H)
...
/mob/proc/update_icons_if_needed()
	...
	if (icon_rebuild_flag & BODY || isAlien(src))
		update_body()
...
/mob/verb/add_memory(msg as message)
	...
	var/message_check = lowertext(msg)
...
/mob/proc/gib(give_medal)
	...
	if (ishuman(src) && !isAlien(src))
		...
		animation = new(src.loc)
		animation.icon_state = "blank"
		animation.icon = 'icons/tg-goon-xenos/xeno.dmi'
		animation.master = src
		flick("gibbed", animation)
	...
	if (client.statpanel == "Status")
		if (ticker)
			if ((!src.client.holder) && (ticker.hide_mode || master_mode == "wizard"))
				...
				var/showmode = master_mode
				if (showmode == "xeno")
					showmode = "Alien Infestation"
				stat("Game Mode:", "[(client.holder && ticker.hide_mode) ? "[showmode] **HIDDEN**" : "[showmode]"]")
...
/mob/proc/choose_name(var/retries = 3)
	...
	for (retries, retries > 0, retries--)
		...
		if (!newname)
			...
			var/message_check = lowertext(newname)
			for (var/v in prohibited_strings)
				if (findtext(message_check, v))
