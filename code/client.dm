...
/client/New()
	...
	spawn(0)
		var/banned = scheck_if_banned()

		if (banned)
			boutput(src, "<span style = \"color:red\"><b>You're [banned]. Head over to https://discord.gg/FmaKmz9 to appeal this. Spamming the channel will upgrade this to a permaban, and not following the format rules will get your appeal promptly removed(and rejected).</span></b>")
			sleep(50)
			if (src)
				del(src)
			return
...
//		return MainframeMove(n,direct,src.mob)
	...
	if (src.mob.canmove)
		...
		if (isturf(src.mob.loc))
			...
			if (!locate(/obj/xeno/hive/weeds) in src.mob.loc)
				src.weed_delay = 0

				if (isAlien(src.mob))
					src.mob:mutantrace:onWeedExit()

			else
				if (!isAlien(src.mob))
					src.weed_delay = 10
				else
					src.mob:mutantrace:onWeedEnter()
			...
			switch(src.mob.m_intent)
				if("run")
					if (src.mob.drowsyness > 0)
						src.move_delay += 6
					src.move_delay += 3.00 + src.weed_delay// was += 2, now += 3
					/*
					if (isAlien(src.mob))
						if (src:mob:mutantrace:leaping)
							src.move_delay -= 1.00
					else
						for (var/obj/item/xeno/facehugger/f in range(src.mob, 10))
							f.onMobSee()
							*/
				if("face")
					src.mob.dir = direct
					return
				if("walk")
					src.move_delay += 8 + src.weed_delay




			if (isAlien(src.mob))
				//slow castes, not intended to chase humans. These should be slower than humans.
				if (istype(src:mob:mutantrace, /datum/mutantrace/xenomorph/drone) && !istype(src:mob:mutantrace, /datum/mutantrace/xenomorph/drone/queen))
					src.move_delay -= 0.05//was 0.2
				else if (istype(src:mob:mutantrace, /datum/mutantrace/xenomorph/sentinel))
					src.move_delay -= 0.06//was 0.22
				else if (istype(src:mob:mutantrace, /datum/mutantrace/xenomorph/praetorian))
					src.move_delay -= 0.04//was 0.18
				else if (istype(src:mob:mutantrace, /datum/mutantrace/xenomorph/drone/queen))
					src.move_delay -= 0.04//was 0.15
				else if (istype(src:mob:mutantrace, /datum/mutantrace/xenomorph/larva) && !istype(src:mob:mutantrace, /datum/mutantrace/xenomorph/larva/facehugger))
					src.move_delay -= 0.05//was 0.15
				//fast castes. These should be faster than humans, but that doesn't require a greater loss of move delay.
				//-0.15 on an xeno has been shown to be faster than -0.35 on the average human.
				else if (istype(src:mob:mutantrace, /datum/mutantrace/xenomorph/warrior))
					src.move_delay -= 0.57//warriors are the fastest, was 0.47
				else if (istype(src:mob:mutantrace, /datum/mutantrace/xenomorph/larva/facehugger))
					src.move_delay -= 0.5//was 0.4
			else
				if (ishuman(src.mob))
					src.move_delay -= 0.6//was 0.5//when this was 0.35 and the queen's speed 0.15, queens could catch up to humans.
					/*   //this implies xeno can catch up to the average human while being 0.2 move delay slower. Tha
			var/move_delay_increase = src.move_delay * (src.speed_mult - 1)
			src.move_delay += move_delay_increase
			*/
...
/*
	if (istype(usr, /client))
		if (usr != src.owner)
			message_admins("<span style=\"color:blue\">[key_name(usr)] has attempted to override client_topic/Topic() for [src.owner]!</span>")
			logTheThing("admin", usr, null, "tried to use [src.owner]'s client_topic/Topic() without authorization.")
			logTheThing("diary", usr, null, "tried to use [src.owner]'s client_topic/Topic() without authorization.", "admin")
			return
	else
...
/client/verb/forum()
	...
	set hidden = 1
	src << link("https://discord.gg/xAjJtPW")