...
/world/proc/load_rules()
	...
	if (!rules)
		rules = file2text("config/rules.txt")
		if (!rules)
			rules = "<html><head><title>Rules</title><body>There are currently no rules.</body></html>"
...
/world/proc/update_status()
	...
	if (!istype(ticker.mode, /datum/game_mode/xeno) && !ALIENMODE)
	...
	//	s += "blame " + svn_author
		...
		s += "<big><b>Horror Station 13 - ALIEN INFESTATION</big></b>"
		s += "<br>"
		s += "("
		s += "<a href=\"https://discord.gg/xAjJtPW\">"
		s += "Discord"
		s += "</a>"
		s += ")"
...
proc/invalid_address(var/address)
	for (var/client/c in world)
		if (c.address == address && !c.holder)
			return 1

	if (address == world.address || findtext(address, "byond://[world.address]") || address == world.internet_address)
		return 0
