...
datum/preferences
	...
	proc/SetChoices(mob/user)
		...
		if (jobban_isbanned(user, "Syndicate"))
			...
			if (src.be_xenomorph) HTML += "<a href =\"byond://?src=\ref[user];preferences=1;b_xenomorph=1\"><font color=#00CC00>Alien</font></a>"
			else HTML += "<a href =\"byond://?src=\ref[user];preferences=1;b_xenomorph=1\"><font color=#FF0000><strike>Alien</strike></font></a>"

			HTML += " * "

			if (src.be_xenomorph)

				if (src.be_xenomorph_queen) HTML += "<a href =\"byond://?src=\ref[user];preferences=1;b_xenomorphq=1\"><font color=#00CC00>Alien Queen</font></a>"
				else HTML += "<a href =\"byond://?src=\ref[user];preferences=1;b_xenomorphq=1\"><font color=#FF0000><strike>Alien Queen</strike></font></a>"
	...
	proc/process_link(mob/user, list/link_tags)
		...
		if (link_tags["b_xenomorph"])
			src.be_xenomorph = !src.be_xenomorph
			src.SetChoices(user)
			return

		if (link_tags["b_xenomorphq"])
			src.be_xenomorph_queen = !src.be_xenomorph_queen
			src.SetChoices(user)
			return
