...
/datum/controller/gameticker/proc/setup()
	...
	if(master_mode in list("secret","wizard"))
		src.hide_mode = 1
	...
	if(!can_continue)
		...
		boutput(world, "<B>Error setting up [modename].</B> Reverting to pre-game lobby.")
	...
	//Distribute jobs
	distribute_jobs()//checked for errors

	//Create player characters and transfer them
	create_characters()//checked for errors

	add_minds()//checked for errors
	...
	//Equip characters
	equip_characters()//checked for errors
		//this happens after creation of xenomorphs, which is very important because survivor info about xenos and shit
		//takes place in jobprocs.dm, procedure alien_mode_spawn_stuff()
	...
	proc/create_characters()
		for(var/mob/new_player/player in mobs)
			...
			if(player.ready)
				...
				else if(player.mind && player.mind.special_role == "xeno")
					player.close_spawn_windows()
					/*
					var/force_queen = 1

					for (var/mob/new_player/somealien in mobs)
						if (somealien != player && somealien.ready && somealien.mind && somealien.client && somealien.client.preferences.be_xenomorph == 1 && somealien.client.preferences.be_xenomorph_queen == 1)
							force_queen = 0//found another queen
							break
							*/

					if (player.mind.assigned_role == "MODEPRIORITYONE")
						player.xenomorphize("Queen", 1)
					else if (player.mind.assigned_role == "MODEPRIORTYTWO")
						player.xenomorphize("Warrior", 1)
					else
						player.xenomorphize("Drone", 1)
/*
					if (player.client.preferences.be_xenomorph_queen == 1 || force_queen)
						if (!got_queen)
							player.make_true_xenomorph(1)
							if (player.client.preferences.be_xenomorph_queen)
								boutput(player, "<span style = \"color:red\"><B>You are as the Xenomorph Queen.</span></B>")
							else if (force_queen)
								boutput(player, "<span style = \"color:red\"><B>Due to a lack of candidates, you have been made the Xenomorph Queen.</span></B>")
							got_queen = 1
						else
							player.make_true_xenomorph(0)
					else
						player.make_true_xenomorph(0)
*/
				else if(player.mind)
	...
	proc/equip_characters()
		for(var/mob/living/carbon/human/player in mobs)
			if(player.mind && player.mind.assigned_role)
				if(!findtext(player.mind.assigned_role, "MODE"))
					spawn(0)
