...
proc/spawn_edible_critters()
	for (var/obj/landmark/l in landmarks)
		if (sthng)
			if (prob(someprob + l.special_critter_chance))//if special critter chance, it means we use l's
			//loc to give survivors their meme messages
				if (critterlen < something)
...
/mob/living/carbon/human/proc/starve()
	...
	if (will_starve_in_deciseconds == -1)
		will_starve_in_deciseconds = rand(3000,6000)

	if (will_starve_in_deciseconds >= 3000  || will_starve_in_deciseconds <= 500)//not hungry or extremely hungry
		var/death_chance = 5

		if (will_starve_in_deciseconds < 500)
			death_chance = 100 - (will_starve_in_deciseconds/10)
			if (death_chance > 100)
				death_chance = 100
			if (death_chance < 50)
				death_chance = 50
