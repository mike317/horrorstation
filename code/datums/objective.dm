...
/datum/objective/regular/assassinate
	...
	proc/find_target_by_role(var/role)
		for(var/datum/mind/possible_target in ticker.minds)
			if((possible_target != owner) && istype(possible_target.current, /mob/living/carbon/human) && (possible_target.assigned_role == role || (findtext(possible_target.assigned_role,"MODE") && possible_target.special_role == role)))
	...
	proc/create_objective_string(var/datum/mind/target)
		...
		var/objective_text = "Assassinate [target.current.real_name], the [findtext(target.assigned_role,"MODE") ? target.special_role : target.assigned_role][add_fluff ? " " : "."]"
		if(add_fluff)
...
proc/create_fluff(var/datum/mind/target)
	...
	var/job = findtext(target.assigned_role, "MODE") ? target.special_role : target.assigned_role
...
/datum/objective/specialist/wraith/murder
	...
	proc/setText()
		explanation_text = "We sense a large untapped astral force with the mortal [target.current.real_name], the [findtext(target.assigned_role, "MODE") ? target.special_role : target.assigned_role]. Trap them in a spiritual form and ensure that they never manifest as a corporeal being again."
		targetname = target.current.real_name
...
/datum/objective/specialist/wraith/murder/absorb
	var/success = 0
	setText()
		explanation_text = "[target.current.real_name], the [findtext(target.assigned_role,"MODE") ? target.special_role : target.assigned_role] is a vessel for astral energies we haven't detected before. Absorb and retain their essence at all costs!"
		targetname = target.current.real_name
...
/datum/objective/escape/survive_xeno
	explanation_text = "Survive the Xenomorph infestation until you are rescued."

	check_completion()
		if(!owner.current || owner.current.stat == 2 || isAlien(owner.current) || !ishuman(owner.current))
			return 0
...
/datum/objective_set/survivor
	objective_list = list()
	escape_choices = list(/datum/objective/escape/survive_xeno)
