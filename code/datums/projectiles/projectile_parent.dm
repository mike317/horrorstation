...
/obj/projectile
	...
	proc/collide(atom/A as mob|obj|turf|area)
		...
		if (immunity || isAlienWarrior(A) && istype(proj_data, /datum/projectile/energy_bolt) && prob(95) || isAlienPraetorian(A) && istype(proj_data, /datum/projectile/laser) && prob(90) || isAlienWarrior(A) && istype(proj_data, /datum/projectile/laser) && prob(70) || isAlienPraetorian(A) && istype(proj_data, /datum/projectile/energy_bolt) && prob(70))
			log_shot(src, A, 1)
	...
	Bump(var/atom/A)
		if (isAlienHugger(A))
			if (prob(90))
				A:visible_message("<span style = \"color:red\">The [src] goes right over [A]'s head!</span>","<span style = \"color:red\">The [src] goes right over your head!</span>")
				return
		src.collide(A)
	...
	Crossed(var/atom/movable/A)
		...
		if (istype(A, /obj/item/xeno/facehugger))
			var/obj/item/xeno/facehugger/f = A
			f.death(0)
			return
