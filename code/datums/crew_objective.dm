...
/datum/controller/gameticker/proc
	...
	generate_individual_objectives(var/datum/mind/crewMind)
		...
		if (!crewMind)
			return
		if (!crewMind.current || !crewMind.objectives || crewMind.objectives.len || crewMind.special_role || (findtext(crewMind.assigned_role, "MODE")))
			return
...
/*
	survivor
		killxeno
			check_completion()
				if (owner.client.xeno_kills && owner.current)
					return 1
				return 0
		...
		killxenos
			var/num
			check_completion()
				if (owner.client.xeno_kills >= num && owner.currnet)
					return 1
				return 0
		...
		leader
			var/percent_needed = rand(30, 70)
			//save 30 to 70% of survivors

		limithive//limit the amount of weed nodes that exist.w

		destroyhive

			caesar//destroy the hive then get killed by your subordinates
		...
		stockpile
			...
			New()
				...
				type = pick(/obj/item/clothing)
				num =

			var/desc_from_type = "of something"
			if (istype(type, /obj/item/clothing))
				desc_from_type = "pieces of clothing"

			explanation_text = "Collect [num] [desc_from_type], and survive with them."

			check_completion()
				var/count = 0
				if (src.current)
					if (src.current.stat == 2)
						return 0
					for (var/obj/o in src.current.contents)
						if (istype(o, type))
							count++
							if (count >= num)
								break
